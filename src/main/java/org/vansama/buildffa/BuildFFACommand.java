...
public class BuildFFACommand implements CommandExecutor {
  ...
  private boolean handleKitEditor(CommandSender sender, String[] args) {
    ...
    if (args.length >= 2) {
      ...
      if (args[1].equalsIgnoreCase("save")) {
        this.kitEditor.saveKit(player);
        return true;
      }
      if (args[1].equalsIgnoreCase("cancel")) {
        this.kitEditor.cancelKit(player);
        return true;
      }
    }
    
    this.kitEditor.openKitEditor(player);
    return true;
  }
  ...
  private boolean handleCreator(CommandSender sender) {
    ...
    sender.sendMessage(colorize("&7Version: &f3.3 (1.8.8)"));
  ...
  private void sendHelp(CommandSender sender) {
    ...
    sender.sendMessage(colorize("&e/buildffa kiteditor &7- Open the Kit Editor"));
    sender.sendMessage(colorize("&e/buildffa kiteditor save &7- Save your kit"));
    sender.sendMessage(colorize("&e/buildffa kiteditor cancel &7- Cancel editing"));
